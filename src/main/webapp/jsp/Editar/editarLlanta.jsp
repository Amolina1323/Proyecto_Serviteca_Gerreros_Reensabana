...
<%@ page import="co.edu.sena.project2687351.model.Llanta" %>
<%
...
<html lang="es"> 
  ...
  <main class="main">
    ...
    <h1>Consulta Especifica</h1>
        ...
        <div class="content">
            <center>
            <form method="POST" action="../../../Project-2687351-1.0-SNAPSHOT/Llanta-Consult">
                <BR>
                <label> Id Llanta :
                <input type="number" name="IdLlanta" class="form-control"><br>
                </label>
            ...
            <% Llanta llanta = (Llanta) request.getAttribute("llanta"); %>
            <!--FORMULARIO DONDE SE MOTRARÁN LOS DATOS ASOCIADOS AL CODIGO DIGITADO-->
            <% if (request.getAttribute("llanta") != null) { %>
...
<!--FORMULARIO DONDE SE MOTRARÁN LOS DATOS ASOCIADOS AL CODIGO DIGITADO-->
            ...
            <center>
                ...
                <BR><BR>
                <label>ID :
                    <input type="number" name="IdLlanta" class="form-control" value="<%= llanta.getIdLlanta() %>" required>
                </label><br><br>
                <label>Marca :
                    <input type="text" name="Marca" class="form-control" value="<%= llanta.getMarca() %>"  required>
            </label><br><br>
                    <label>Tipo : <br><br>
                        <select name="Tipo">
                            <option value="Liviano" <%= llanta.getTipo().equals("Liviano") ? "selected" : "" %>>Liviano</option>
                            <option value="Mediano" <%= llanta.getTipo().equals("Mediano") ? "selected" : "" %>>Mediano</option>
                            <option value="Pesado" <%= llanta.getTipo().equals("Pesado") ? "selected" : "" %>>Pesado</option>
                        </select>
                    </label><br><br>
            <label>Modelo :
            <input type="text" name="Modelo" class="form-control" value="<%= llanta.getModelo() %>"  required>
            </label><br><br>
            <label>Dimension  :
            <input type="text" name="Dimension" class="form-control" value="<%= llanta.getDimension() %>"  required>
            </label><br><br>
            <label>Diseño    :
            <input type="text" name="Diseno" class="form-control" value="<%= llanta.getDiseno() %>"  required>
            </label><br><br>
            <label>FechaLlanta   :
                <input type="date" name="FechaLlanta" class="form-control" value="<%= llanta.getFechaLlanta() %>"  required>
                </label><br><br>
            <label>Profundidad Llanta    :
            <input type="number" name="ProfundidadLlanta" class="form-control" value="<%= llanta.getProfundidadLlanta() %>"  required>
            </label><br><br>
                    <% if (rol == 1 || rol == 2) {%>
                    <div class="grupo boton animacion" >
                        <br>
            ...
            <script>
                function editar() {
                    ...
                    if(rest == true){
                        document.getElementById("Formulario").action = "../../../Project-2687351-1.0-SNAPSHOT/Llanta-Update";
                ...
                function eliminar() {
                    ...
                    if(rest == true){
                        document.getElementById("Formulario").action = "../../../Project-2687351-1.0-SNAPSHOT/Llanta-Delete";
                        document.getElementById("Formulario").submit();
                    }else {
